...
export const getApplicationRisk = async (applicationId: number | string) => {
  const res = await api.get<ApiResponse<any>>(`/risk/application/${applicationId}`);
  return res.data?.data;
};

export const simulateWhatIf = async (
  businessId: number | string,
  options?: { resolve_missing_documents?: boolean; resolve_document_errors?: boolean; resolve_dependencies?: boolean }
) => {
  const res = await api.post<ApiResponse<any>>(`/risk/what-if/${businessId}`, options || {});
  return res.data?.data;
};
